# 1 "Uds_server/JumpToAPP.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 379 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Uds_server/JumpToAPP.c" 2
# 33 "Uds_server/JumpToAPP.c"
 ...
 void app_jump(uint32_t user_jump_flag)
{
  ...
  {
   Address=(uint32*)(0x12002000 + 0x100U);
  }
  else if(user_jump_flag==0xdd)
  {
   Address=(uint32*)( 0x11000000 + 0x100U);
  }
  else
  {
   Address=(uint32*)(0x12002000 + 0x100U);
  }
  if(Address[0] >= ((0x18000000U)) &&
   Address[0] < ((0x18002000U) + (0x5C00U)) &&
   Address[1] >= ((0x12002000U)) &&
   Address[1] < ((0x12002000U) + (0x3E000U)))
    {

        PMU_serviceFailSafeWatchdogSOW();
        ((GPIO_Type*) 0x48030000UL)->P1_OMR.reg = 0x00010001;


        __disable_irq();


        ((CPU_Type*) 0xE000E000UL)->VTOR.reg = (0x12002000 + 0x100U);
