...
static const IfxSmu_Alarm g_smuAlarms_TC375_clear[] =
{
        IfxSmu_Alarm_CPU0_Lockstep_ComparatorError                              ,
        IfxSmu_Alarm_CPU0_Buslevel_MpuViolation                                 ,
        IfxSmu_Alarm_CPU0_PFI0_PFLASH0_ReadPathError                            ,
...
void Smu_Init(void)
{
    ...
    for(uint32 i = 0; i < (sizeof(g_smuAlarms_TC375_clear)/sizeof(g_smuAlarms_TC375_clear[0])); i++)
    {
        IfxSmu_clearAlarmStatus(g_smuAlarms_TC375_clear[i]);
    }
    for(uint32 i = 0; i < (sizeof(g_smuAlarms_TC375_clear)/sizeof(g_smuAlarms_TC375_clear[0])); i++)
    {
        IfxSmu_setAlarmAction(g_smuAlarms_TC375_clear[i], IfxSmu_InternalAlarmAction_nmi);
    }
