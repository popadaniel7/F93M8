#define LCF_CSA0_SIZE 350
#define LCF_USTACK0_SIZE 3k
#define LCF_ISTACK0_SIZE 3k

#define LCF_CSA1_SIZE 350
#define LCF_USTACK1_SIZE 3k
#define LCF_ISTACK1_SIZE 3k

#define LCF_CSA2_SIZE 350
#define LCF_USTACK2_SIZE 3k
#define LCF_ISTACK2_SIZE 3k
...
#define LCF_STARTPTR_CPU0    0x80000000 // 0x80060000
#define LCF_STARTPTR_CPU1    0x80000200
#define LCF_STARTPTR_CPU2    0x80000220

#define LCF_TRAPVEC0_START   0x80001000 
#define LCF_TRAPVEC1_START   0x80002000 
#define LCF_TRAPVEC2_START   0x80004000  

#define LCF_INTVEC0_START    0x8000F000 
#define LCF_INTVEC1_START    0x8000F100  
#define LCF_INTVEC2_START    0x8000F200  
...
#        endif
    ...
    section_layout :vtc:linear
    {
        ...
        group
        {
            ...
            group
            {
                group (ordered, attributes = rw, run_addr = mem:dsram2, align = 4)
                {
                ...
                {
                    group ASILD_RAM (ordered, attributes = rw, align = 4)
                    {
                        ...
                        group EPSCTRL (ordered, attributes = rw, align = 4)
                        {
                            select ".data.EPSCtrl.*";
                            select ".bss.EPSCtrl.*";
                            select ".zdata.EPSCtrl.*";
                            select ".zbss.EPSCtrl.*";
                        }
                        "__EPSCTRL_START" := "_lc_gb_EPSCTRL";
                        "__EPSCTRL_END"   := "_lc_ge_EPSCTRL";
                    ...
                    "__ASILDRAM_START" := "_lc_gb_ASILD_RAM";
                    "__ASILDRAM_END"  := "_lc_ge_ASILD_RAM";
                    
                    group ASILC_RAM (ordered, attributes=rw, align=4)
                    {           
                        ...
                        group EMOTORCTRL (ordered, attributes = rw, align = 4)
                        {
                            select ".data.EMotorCtrl.*";
                            select ".bss.EMotorCtrl.*";
                            select ".zdata.EMotorCtrl.*";
                            select ".zbss.EMotorCtrl.*";
                        }
                        "__EMOTORCTRL_START" := "_lc_gb_EMOTORCTRL";
                        "__EMOTORCTRL_END"   := "_lc_ge_EMOTORCTRL";
                    }
                    "__ASILCRAM_START" := "_lc_gb_ASILC_RAM";
                    "__ASILCRAM_END"   := "_lc_ge_ASILC_RAM";
                    
                    group ASILB_RAM (ordered, attributes = rw, align = 4)
                    {
                    ...
                    "__ASILBRAM_START" := "_lc_gb_ASILB_RAM";
                    "__ASILBRAM_END"   := "_lc_ge_ASILB_RAM";
                    
                    group QM_RAM (ordered, attributes = rw, align = 4)
                    {
                        group ENERGYMGMT (ordered, attributes = rw, align = 4)
                        {
                        ...
                        "__ENERGYMGMT_START" := "_lc_gb_ENERGYMGMT";
                        "__ENERGYMGMT_END"   := "_lc_ge_ENERGYMGMT";
                        ...
                        group DEM (ordered, attributes = rw, align = 4)
                        {
                            select ".data.Dem.*";
                            select ".bss.Dem.*";
                            select ".zdata.Dem.*";
                        ...
                        "__DEM_START" := "_lc_gb_DEM";
                        "__DEM_END"   := "_lc_ge_DEM";
                    }
                    "__ASILBRAM_START" := "_lc_gb_ASILB_RAM";
                    "__ASILBRAM_END"   := "_lc_ge_ASILB_RAM";
                }
